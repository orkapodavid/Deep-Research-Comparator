...
async def stream_perplexity_api(
    model: str | None = None, user_message: str = "", system_message: str | None = None
):
    """
        ...
        yield json.dumps(
            {
                ...
                "model_name": model or DEFAULT_PPLX_MODEL_NAME,
            }
        )
        return
    ...
    if not user_message:
        logger.warning(
            f"User message is empty for Perplexity API call to model {actual_model}."
        )
    ...
    try:
                ...
                async for line in response.aiter_lines():
                    if line and line.startswith("data: "):
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to decode JSON from Perplexity stream: {event_data}"
                            )
                            continue  # Skip malformed lines
                        except Exception as e:
                            logger.error(
                                f"Error processing chunk from Perplexity for model {actual_model}: {e}, data: {event_data}"
                            )
                            # Yield an error structure if a specific chunk fails mid-stream
                            yield {
                                "error": f"Chunk processing error: {str(e)}",
                                "model_name": actual_model,
                                "original_chunk": event_data,
                            }
                            continue
        ...
        logger.error(
            f"Perplexity API request failed for model {actual_model} with status {e.response.status_code}: {error_detail}"
        )
        yield json.dumps(
            {
        ...
        logger.error(
            f"RequestError connecting to Perplexity API for model {actual_model}: {e}"
        )
        yield json.dumps(
            {
        ...
        logger.exception(
            f"Unexpected error in stream_perplexity_api for model {actual_model}:"
        )  # Logs with stack trace
        yield json.dumps(
            {
                "error": f"Unexpected error streaming from Perplexity: {type(e).__name__} - {str(e)}",
                "model_name": actual_model,
            }
        )
