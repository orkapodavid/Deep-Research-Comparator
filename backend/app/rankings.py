...
import numpy as np 
from db_schema import DeepResearchUserResponse, AnswerSpanVote,IntermediateStepVote, DeepresearchRankings, DeepResearchAgent
...
def return_bt_dataframe():
    rows = session.query(DeepResearchUserResponse.agentid_a, DeepResearchUserResponse.agentid_b, DeepResearchUserResponse.userresponse).all()
    updated_rows = []
    for row in rows:
        if row[2]=="choice1":
...
def compute_Answering_Span_Upvote_rate(agent_id):
    count_ans = session.query(AnswerSpanVote.agent_id, AnswerSpanVote.vote).filter(AnswerSpanVote.agent_id==agent_id).all()
    rows_up = session.query(AnswerSpanVote.agent_id, AnswerSpanVote.vote).filter(
    and_(
        AnswerSpanVote.agent_id == agent_id,
        AnswerSpanVote.vote == "up"
    )
).all()

    rows_down = session.query(AnswerSpanVote.agent_id, AnswerSpanVote.vote).filter(
    and_(
        AnswerSpanVote.agent_id == agent_id,
        AnswerSpanVote.vote == "down"
    )
).all()
...
def compute_Step_Upvote_Rate(agent_id):
    count_ans = session.query(IntermediateStepVote.agent_id, IntermediateStepVote.vote).filter(IntermediateStepVote.agent_id==agent_id).all()
    rows_up = session.query(IntermediateStepVote.agent_id, IntermediateStepVote.vote).filter(
    and_(
    ...
    rows_down = session.query(IntermediateStepVote.agent_id, IntermediateStepVote.vote).filter(
    and_(
        IntermediateStepVote.agent_id == agent_id,
...
def calculate_total_votes(agent_id):
    total_votes = 0
    rows1 = session.query(DeepResearchUserResponse.agentid_a, DeepResearchUserResponse.agentid_b, 
                                DeepResearchUserResponse.userresponse).filter(
                and_(
    ...
    rows2 = session.query(DeepResearchUserResponse.agentid_a, DeepResearchUserResponse.agentid_b, 
                                DeepResearchUserResponse.userresponse).filter(
                and_(
...
def return_system_name(agent_id):
    rows = session.query(DeepResearchAgent.agent_name).filter(
...
def insert_into_rankings(sorted_rankings_object_array):
    for obj in sorted_rankings_object_array:
        rows = session.query(DeepresearchRankings.SYSTEMID).filter(
